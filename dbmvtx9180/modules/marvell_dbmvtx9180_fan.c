...
static struct attribute *dbmvtx9180_fan_attributes[] = {
...
static ssize_t get_version(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	struct dbmvtx9180_fan_data     *data = i2c_get_clientdata(client);
	int                             version;
...
static const struct attribute_group dbmvtx9180_fan_group = {
    .attrs = dbmvtx9180_fan_attributes,
};

static int dbmvtx9180_fan_probe
(
    struct i2c_client *client,
    const struct i2c_device_id *dev_id
)
{
    struct dbmvtx9180_fan_data  *data;
    int                         status;

    data = kzalloc(sizeof(struct dbmvtx9180_fan_data), GFP_KERNEL);
    if (!data)
    {
        status = -ENOMEM;
        goto exit;
    ...
    status = sysfs_create_group(&client->dev.kobj, &dbmvtx9180_fan_group);
    if (status)
    {
        goto exit_free;
    }

	data->hwmon_dev = hwmon_device_register_with_info(&client->dev, "marvell_fan", NULL, NULL, NULL);
...
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &dbmvtx9180_fan_group);
exit_free:
    kfree(data);
exit:
...
static int dbmvtx9180_fan_remove(struct i2c_client *client)
{
    struct dbmvtx9180_fan_data *data = i2c_get_clientdata(client);
...
static const struct i2c_device_id dbmvtx9180_fan_id[] = {
    { "dbmvtx9180_fan", 0 },
    {}
};
MODULE_DEVICE_TABLE(i2c, dbmvtx9180_fan_id);

static struct i2c_driver dbmvtx9180_fan_driver = {
    .class        = I2C_CLASS_HWMON,
    .driver = {
        .name     = DRVNAME,
    },
    .probe        = dbmvtx9180_fan_probe,
    .remove       = dbmvtx9180_fan_remove,
    .id_table     = dbmvtx9180_fan_id,
    .address_list = normal_i2c,
};

module_i2c_driver(dbmvtx9180_fan_driver);

MODULE_AUTHOR("Krishnan Sridharan <ksridharan@marvell.com>");
MODULE_DESCRIPTION("dbmvtx9180_fan driver");
MODULE_LICENSE("GPL");

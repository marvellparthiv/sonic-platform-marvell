/*
 * An hwmon driver for the marvell Redundant Power Module
 *
 */
...
#define DRIVER_DESCRIPTION_NAME     "marvell i2c psu driver"
...
enum marvell_i2c_psu_sysfs_attributes {
    PSU_V_IN,
    PSU_V_OUT,
    PSU_I_IN,
    PSU_I_OUT,
...
static struct attribute *marvell_i2c_psu_attributes[] = {
...
static ssize_t get_value(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    struct marvell_i2c_psu_data     *data = i2c_get_clientdata(client);
    size_t                          ret_count=0;
...
static const struct attribute_group marvell_i2c_psu_group = {
    .attrs = marvell_i2c_psu_attributes,
};

static int marvell_i2c_psu_probe(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
    struct marvell_i2c_psu_data *data;
    int                         status;

    data = kzalloc(sizeof(struct marvell_i2c_psu_data), GFP_KERNEL);
    if (!data)
    {
        status = -ENOMEM;
        goto exit;
    ...
    status = sysfs_create_group(&client->dev.kobj, &marvell_i2c_psu_group);
    if (status)
    {
        goto exit_free;
    }

	data->hwmon_dev = hwmon_device_register_with_info(&client->dev, "marvell_i2c_psu", NULL, NULL, NULL);
...
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &marvell_i2c_psu_group);
exit_free:
    kfree(data);
exit:
...
static int marvell_i2c_psu_remove(struct i2c_client *client)
{
    struct marvell_i2c_psu_data *data = i2c_get_clientdata(client);
...
/* Support psu moduel
 */
static const struct i2c_device_id marvell_i2c_psu_id[] = {
    { "acbel_fshxxx", 0 },
    {}
};
MODULE_DEVICE_TABLE(i2c, marvell_i2c_psu_id);

static struct i2c_driver marvell_i2c_psu_driver = {
    .class        = I2C_CLASS_HWMON,
    .driver = {
        .name     = "marvell_i2c_psu",
    },
    .probe        = marvell_i2c_psu_probe,
    .remove       = marvell_i2c_psu_remove,
    .id_table     = marvell_i2c_psu_id,
    .address_list = normal_i2c,
};

static int __init marvell_i2c_psu_init(void)
{
    return i2c_add_driver(&marvell_i2c_psu_driver);
}

static void __exit marvell_i2c_psu_exit(void)
{
    i2c_del_driver(&marvell_i2c_psu_driver);
}
...
module_init(marvell_i2c_psu_init);
module_exit(marvell_i2c_psu_exit);

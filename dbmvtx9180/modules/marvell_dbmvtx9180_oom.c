...
static struct attribute *dbmvtx9180_oom_attributes[] = {
...
static ssize_t get_oom_value(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    struct dbmvtx9180_oom_data      *data = i2c_get_clientdata(client);
    int                             value = 0;
...
static const struct attribute_group dbmvtx9180_oom_group = {
    .attrs = dbmvtx9180_oom_attributes,
};

static int dbmvtx9180_oom_probe(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
    struct dbmvtx9180_oom_data *data;
    int status;

    data = kzalloc(sizeof(struct dbmvtx9180_oom_data), GFP_KERNEL);
    if (!data)
    {
        status = -ENOMEM;
        goto exit;
    ...
    status = sysfs_create_group(&client->dev.kobj, &dbmvtx9180_oom_group);
    if (status)
    {
        goto exit_free;
    }

	data->hwmon_dev = hwmon_device_register_with_info(&client->dev, "marvell_oom", NULL, NULL, NULL);
...
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &dbmvtx9180_oom_group);
exit_free:
    kfree(data);
exit:
...
static int dbmvtx9180_oom_remove(struct i2c_client *client)
{
    struct dbmvtx9180_oom_data *data = i2c_get_clientdata(client);
...
static const struct i2c_device_id dbmvtx9180_oom_id[] = {
    { "dbmvtx9180_oom", 0 },
    {}
};
MODULE_DEVICE_TABLE(i2c, dbmvtx9180_oom_id);

static struct i2c_driver dbmvtx9180_oom_driver = {
    .class        = I2C_CLASS_HWMON,
    .driver = {
        .name     = "dbmvtx9180_oom",
    },
    .probe        = dbmvtx9180_oom_probe,
    .remove       = dbmvtx9180_oom_remove,
    .id_table     = dbmvtx9180_oom_id,
    .address_list = normal_i2c,
};

static int __init dbmvtx9180_oom_init(void)
{
    return i2c_add_driver(&dbmvtx9180_oom_driver);
}

static void __exit dbmvtx9180_oom_exit(void)
{
    i2c_del_driver(&dbmvtx9180_oom_driver);
}

module_init(dbmvtx9180_oom_init);
module_exit(dbmvtx9180_oom_exit);

MODULE_AUTHOR("Krishnan Sridharan <ksridharan@marvell.com>");
MODULE_DESCRIPTION("dbmvtx9180_oom driver");
MODULE_LICENSE("GPL");

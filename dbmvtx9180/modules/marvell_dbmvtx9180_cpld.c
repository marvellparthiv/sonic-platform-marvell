...
#define PORT_NUM               32
#define SYSLED_NUM             5
#define DBMVTX9180_FPGA_ADDR   0x19
...
static const struct i2c_device_id dbmvtx9180_cpld_id[] = {
    { "dbmvtx9180_fpga", dbmvtx9180_fpga },
    { "dbmvtx9180_cpld1", dbmvtx9180_cpld1 },
    { }
};
MODULE_DEVICE_TABLE(i2c, dbmvtx9180_cpld_id);

#define TRANSCEIVER_PRESENT_ATTR_ID(index)      MODULE_PRESENT_##index
#define TRANSCEIVER_RESET_ATTR_ID(index)        MODULE_RESET_##index

enum dbmvtx9180_cpld_sysfs_attributes {
    /* chip version */
    CPLD_VERSION,
    /* system led */
    SYSTEM_LED_LOC,
...
static struct attribute *dbmvtx9180_fpga_attributes[] = {
...
static const struct attribute_group dbmvtx9180_fpga_group = {
    .attrs = dbmvtx9180_fpga_attributes,
};

static struct attribute *dbmvtx9180_cpld1_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
...
static const struct attribute_group dbmvtx9180_cpld1_group = {
    .attrs = dbmvtx9180_cpld1_attributes,
};
...
static ssize_t get_status(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    struct dbmvtx9180_cpld_data     *data = i2c_get_clientdata(client);
...
static void dbmvtx9180_cpld_add_client(struct i2c_client *client)
{
...
static void dbmvtx9180_cpld_remove_client(struct i2c_client *client)
{
 ...
 */
static int dbmvtx9180_cpld_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    struct dbmvtx9180_cpld_data     *data;
    int                             ret = -ENODEV;
    const struct attribute_group    *group = NULL;

    data = kzalloc(sizeof(struct dbmvtx9180_cpld_data), GFP_KERNEL);
    if (!data)
    {
        ret = -ENOMEM;
        goto exit;
    ...
    switch (data->type)
    {
        case dbmvtx9180_fpga:
            group = &dbmvtx9180_fpga_group;
            break;
        case dbmvtx9180_cpld1:
            group = &dbmvtx9180_cpld1_group;
            break;
        default:
            break;
    }
    ...
    dbmvtx9180_cpld_add_client(client);
    return 0;
...
static int dbmvtx9180_cpld_remove(struct i2c_client *client)
{
    struct dbmvtx9180_cpld_data *data = i2c_get_clientdata(client);
    const struct attribute_group *group = NULL;

    dbmvtx9180_cpld_remove_client(client);
...
int dbmvtx9180_fpga_sysled_get(int led_type)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EPERM;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == DBMVTX9180_FPGA_ADDR) {
...
EXPORT_SYMBOL(dbmvtx9180_fpga_sysled_get);

int dbmvtx9180_fpga_sysled_set(int led_type, int light_mode)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EIO;
...
EXPORT_SYMBOL(dbmvtx9180_fpga_sysled_set);

static struct i2c_driver dbmvtx9180_cpld_driver = {
    .driver     = {
        .name   = "dbmvtx9180_cpld",
        .owner  = THIS_MODULE,
    },
    .probe      = dbmvtx9180_cpld_probe,
    .remove     = dbmvtx9180_cpld_remove,
    .id_table   = dbmvtx9180_cpld_id,
};

static int __init dbmvtx9180_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&dbmvtx9180_cpld_driver);
}

static void __exit dbmvtx9180_cpld_exit(void)
{
    i2c_del_driver(&dbmvtx9180_cpld_driver);
}

MODULE_AUTHOR("Krishnan Sridharan <ksridhran@marvell.com>");
MODULE_DESCRIPTION("Marvell I2C CPLD driver");
MODULE_LICENSE("GPL");

module_init(dbmvtx9180_cpld_init);
module_exit(dbmvtx9180_cpld_exit);
